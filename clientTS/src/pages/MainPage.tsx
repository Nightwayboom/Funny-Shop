...
import type { newCardType, UserProps } from '../components/app.type';
...
function MainPage({ user }: UserPropsWithoutSetUser) : JSX.Element {
  ...
  return (
    <>
      ...
      >
        {cards &&
          cards.map((card) => (
            <ShopCard key={card.id} card={card} cards={cards} setCards={setCards} />
          ))}
      </div>
    </>
  );
